...
import type { AdapterInstance, betterAuth } from "better-auth";
...
import type defaultSchema from "../component/schema";
    ...
    registerRoutes: (
      ...
      const cors = corsRouter(http, {
        allowedOrigins: async (request) => {
          ...
          const cleanedTrustedOrigins = (trustedOrigins ?? []).filter(
            (origin): origin is string => typeof origin === "string" && origin.length > 0
          );
          const normalizedTrustedOrigins = cleanedTrustedOrigins
            .map((origin) =>
          ...
          const cleanedCorsAllowedOrigins = (corsOpts.allowedOrigins ?? []).filter(
            (origin): origin is string => typeof origin === "string" && origin.length > 0
          );
          return normalizedTrustedOrigins.concat(cleanedCorsAllowedOrigins);
        },
